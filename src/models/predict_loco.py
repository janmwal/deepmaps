...
def leave_one_center_out(table, train_center):
    ...
    test_ids = table[(table['center'] != train_center) & (table['noisy'] == False)].index.tolist()
    #print(train_ids)
    #print(test_ids)
    return train_ids, test_ids
...
def predict(args):
    ...
    for n_center, center in enumerate(tqdm(centers, 
                          colour = 'blue', 
                          desc = 'Center', 
                          position = 0, 
                          leave = False)):
        ...
        train_valid_ids, test_ids = leave_one_center_out(table, train_center=center)
        ...
        with torch.no_grad():
            ...
            for test_batch, (images, labels_, indices) in enumerate(test_loader):
                ...
                for i in range(len(indices)):
                    table.loc[indices[i],'prediction_rounded'] = outputs_rounded[i].detach().cpu().numpy().astype(int)
                    table.loc[indices[i],'prediction'] = outputs[i].detach().cpu().numpy().astype(float)
    ...
    path = joinfile(MODELS_DIR, f'predict_loco/{model_class_name}{version}/datasets/{dt_string}.csv')
    Path(path).parents[0].mkdir(parents=True, exist_ok=True)
    dataset.export_df(path)

    path = joinfile(MODELS_DIR, f'predict_loco/{model_class_name}{version}/perf/{dt_string}.csv')
    Path(path).parents[0].mkdir(parents=True, exist_ok=True)

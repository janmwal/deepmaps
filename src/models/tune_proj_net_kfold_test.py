...
from optuna.pruners import ThresholdPruner, NopPruner
import torch
...
def tune():
    ...
    pruner = NopPruner() if OPTIMIZE_LOSS else ThresholdPruner(lower=0.4)
    study = optuna.create_study(
        direction = direction, 
        study_name = study_name, 
        pruner = pruner,
        storage='sqlite:///projnet.db',
        load_if_exists=True)
    try:
    ...
    path = joinfile(MODELS_DIR, f'Optuna/ProjNet/Merged1/{timestamp}.csv')
